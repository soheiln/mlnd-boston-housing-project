...
def explore_city_data(city_data):
	...
	data_size = len(housing_prices)
	feature_size = len(housing_features[0])
	min_value = np.min(housing_prices)
	max_value = np.max(housing_prices)
...
def performance_metric(label, prediction):
	...
	score = metrics.mean_squared_error(label,prediction)
	return score
